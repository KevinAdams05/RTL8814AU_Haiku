...
| [reason-15-investigation.md](reason-15-investigation.md) | The open defect's full record: every hypothesis measured and killed, with what each cost. Read before proposing an experiment — re-running a dead one is the easiest way to lose a session |
| [solved-defects.md](solved-defects.md) | Faults that are fixed, and the evidence that settled them — including explanations this project once led with and later disproved. Read before re-opening anything on a hunch |
